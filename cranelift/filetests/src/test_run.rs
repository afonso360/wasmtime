...
use cranelift_codegen::settings::{Configurable, SettingKind};
...
fn is_isa_compatible(
    ...
    let bool_isa_flags = requested_flags
        .iter()
        .filter(|v| v.kind() == SettingKind::Bool);
    for req_value in bool_isa_flags {
        let requested = req_value.as_bool().unwrap_or(false);
