...
mod tests {
    ...
    fn basic_data_context() {
        ...
        let _func_a = data_ctx.import_function(ModuleRelocTarget::user(0, 0));
        let func_b = data_ctx.import_function(ModuleRelocTarget::user(0, 1));
        let func_c = data_ctx.import_function(ModuleRelocTarget::user(0, 2));
        let _data_a = data_ctx.import_global_value(ModuleRelocTarget::user(0, 3));
        let data_b = data_ctx.import_global_value(ModuleRelocTarget::user(0, 4));
